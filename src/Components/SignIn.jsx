...
export default function SignInForm() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      // ✅ If user is not verified, redirect them to OTP page
      if (err.response?.status === 403 && serverMsg.toLowerCase().includes("verify")) {
        const emailToVerify = identifier;
        localStorage.setItem("verif_email", emailToVerify);
