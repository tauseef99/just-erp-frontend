...
const ProfileImageUpload = ({ currentImage, onImageUpdate }) => {
  ...
  const handleImageUpload = async (event) => {
    ...
    try {
      ...
      const response = await axios.post(
        "${process.env.REACT_APP_API_URL}/api/seller/profile/upload-image",
        formData,
        {
          headers: {
  ...
  {currentImage ? (
    <img
      src={`${process.env.REACT_APP_API_URL}/uploads/${currentImage}`}
      alt="Profile"
...
export default function SellerProfile() {
  ...
  const fetchSellerData = async () => {
    try {
      ...
      const response = await axios.get('${process.env.REACT_APP_API_URL}/api/profile-wizard', {
        headers: { 
          Authorization: `Bearer ${cleanToken}`,
      ...
      try {
        const profileResponse = await axios.get('${process.env.REACT_APP_API_URL}/api/seller/profile', {
          headers: { 
            Authorization: `Bearer ${cleanToken}`,
            'Content-Type': 'application/json'
          }
  ...
  const handleSaveSection = async (updatedData) => {
    try {
      ...
      if (step !== null) {
        await axios.put('${process.env.REACT_APP_API_URL}/api/profile-wizard/step', {
          step,
          data: dataToSend,
          isCompleted: true
        }, {
