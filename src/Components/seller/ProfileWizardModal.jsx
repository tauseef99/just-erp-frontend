...
export default function ProfileWizardModal({ 
    ...
    const validateToken = async () => {
      ...
      if (token) {
        try {
          ...
          await axios.get('${process.env.REACT_APP_API_URL}/api/seller/profile', {
  ...
  const saveStepData = async (step) => {
    try {
      ...
      const response = await axios.put('${process.env.REACT_APP_API_URL}/api/profile-wizard/step', {
        step,
        data: stepData,
        isCompleted: true
      }, {
    ...
    try {
      ...
      if (currentStep < steps.length - 1) {
        ...
        try {
          ...
          const response = await axios.post('${process.env.REACT_APP_API_URL}/api/profile-wizard/tagline', {}, {
            headers: { 
              Authorization: `Bearer ${cleanToken}`,
              'Content-Type': 'application/json'
            }
